...
from logging import (CRITICAL, DEBUG, ERROR, FileHandler, Formatter, Handler, INFO, Logger, LoggerAdapter,
                     StreamHandler, WARNING, getLogger, NOTSET)
...
class EndpointLoggerAdapter(SuppressedLoggerAdapter):
    """
    Logger adapter for logging requests to an endpoint.
    """
    ...
    def logRequest(self, request: Request):
        ...
        self.log(
            INFO,
            f"Request from {request.client.host} to {request.url} with method {request.method}"  # TODO: Figure out what class the user is
        )
